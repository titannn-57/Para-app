...
export async function getAICoachResponseAction(userId: string, message: string) {
  try {
    ...
    if (challenge) {
      ...
      completedTasksCount = allTasks.filter((task: any) => task.completed).length
      ...
      // Get today's tasks
      todaysTasks = allTasks.filter((task: any) => task.day === challenge.currentDay)
    }
    ...
    if (todaysTasks.length > 0) {
      ...
      todaysTasks.forEach((task: any, index: number) => {
